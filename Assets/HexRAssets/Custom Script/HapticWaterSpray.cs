...
namespace Oculus.Interaction.Demo
{
    public class HapticWaterSpray : MonoBehaviour, IHandGrabUseDelegate
    {
        ...
        {
            if(strength > 0.5 && PressureIncreased == false)
            {
                //Haptics control trigger index finger by 30 pressure
                PressureIncreased = true;
                RightpressureTrackerMain.TriggerSingleHapticsIncrease(new byte[] { 1, 0 },30, false);
                LeftpressureTrackerMain.TriggerSingleHapticsIncrease(new byte[] { 1, 0 }, 30, false);
            }
            else if(strength <= 0.5 && PressureIncreased == true)
            {
                //Haptics control remove index finger by 30 pressure
                PressureIncreased = false;
                RightpressureTrackerMain.TriggerSingleHapticsIncrease(new byte[] { 1, 2 }, 30, false);
                LeftpressureTrackerMain.TriggerSingleHapticsIncrease(new byte[] { 1, 2 }, 30, false);
            }
